<div class="py-10 bg-gray-100 screen">
    <h2 class="sr-only">Edit post</h2>
    <div class="flex justify-between pb-2 border-b">
        <h3 class="text-lg font-medium text-gray-900">{{ __('Edit Post') }}</h3>
        <a href="{{ route('blog.posts') }}" class="btn-cancel">
            {{ __('Back') }}
        </a>
    </div>
    <form id="update_post_form" class="relative" wire:submit.prevent="update">
                ...
                <div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8">
                    ...
                    <div class="sm:col-span-2">
                        <div wire:ignore>
                            ...
                            {{-- </div> --}}
                            <div class="mt-1">
                                <textarea x-data x-ref='ckEditorEN' x-init="ClassicEditor
                                    .create($refs.ckEditorEN, {
                                        language: 'en'
                                    }).then(function(editor) {
                                        editor.model.document.on('change:data', () => {
                                            @this.set('post_content_en', editor.getData());
                                        });
                                    })
                                    .catch(error => {

                                    });" class="resize-none" rows="10">
                                    {{ $content_en }}
                            </textarea>
                            </div>
                        </div>
                    ...
                    <div class="sm:col-span-2">
                        <div wire:ignore>
                            ...
                            {{-- </div> --}}
                            <div class="mt-1">
                                <textarea x-data x-ref='ckEditorAR' x-init="ClassicEditor
                                    .create($refs.ckEditorAR, {
                                        language: 'id'
                                    }).then(function(editor) {
                                        editor.model.document.on('change:data', () => {
                                            @this.set('post_content_id', editor.getData());
                                        });
                                    })
                                    .catch(error => {
                    ...
                    <div class="flex flex-col">
                        @if ($image && is_string($image))
                            <div
                                ...
                                <div
                                    class="relative flex flex-col items-center justify-center w-full h-60 overflow-hidden text-center bg-gray-100 border rounded cursor-pointer select-none">
                                    <button
                                    ...
                                    <div>
                                        <img class="absolute inset-0 z-0 object-fill w-full h-60 border-4 border-white"
                                            src="{{ asset($image) }}" />
                                    </div>
